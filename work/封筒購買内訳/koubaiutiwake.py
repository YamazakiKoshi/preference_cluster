...
# 購買内訳を 0.0 から 1.0 に変換
max_values = result_df.iloc[:, 1:].max(axis=1)  # 各行の最大値を取得
result_df.iloc[:, 1:] = result_df.iloc[:, 1:].div(max_values, axis=0)


# 顧客ごとに統合
result_df.to_csv('./work/data/feature/utiwake.csv', index=False, encoding='utf-8')
print(result_df)