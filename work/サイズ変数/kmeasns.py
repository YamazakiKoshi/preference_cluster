...
# データを読み込む
df = pd.read_csv('./work/data/feature/size_kikaku.csv')
df = df.drop('得意先コード', axis=1)
# # データを読み込む
# df = pd.read_excel('因子得点.xlsx')
# 標準化器を作成して標準得点を計算
...
# クラスタごとの統計的な特性を表示
cluster_statistics = df.groupby('cluster').describe()
print("クラスタごとの統計的な特性:")
print(cluster_statistics)
# データフレームに変換
cluster_statistics = pd.DataFrame(cluster_statistics)
# Excelファイルに保存
excel_file_path = "統計的な特性.xlsx"
cluster_statistics.to_excel(excel_file_path, index=True)
