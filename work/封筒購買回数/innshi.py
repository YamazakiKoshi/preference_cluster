...
# Excelファイルに保存
excel_file_path = "購買固有値.xlsx"
df_factor_loadings.to_excel(excel_file_path, index=False)


# 因子分析の実行
fa = FactorAnalyzer(n_factors=9,rotation="oblimin",method="ml")
fa.fit(df_workers_std)

# 因子負荷量，共通性
loadings_df = pd.DataFrame(fa.loadings_, columns=["因子1", "因子2", "因子3", "因子4", "因子5", "因子6", "因子7", "因子8", "因子9"])
...
factor_correlation_df = pd.DataFrame(factor_correlation, 
                                     columns=["因子1", "因子2", "因子3", "因子4", "因子5", "因子6", "因子7", "因子8", "因子9"], 
                                     index=["因子1", "因子2", "因子3", "因子4", "因子5", "因子6", "因子7", "因子8", "因子9"])

# 結果をExcelファイルに保存
excel_file_path_correlation = "購買因子間相関.xlsx"
...
df_factor_only.to_excel("購買因子得点.xlsx", index=False)
